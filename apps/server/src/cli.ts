...
const GITHUB_RELEASES_URL = 'https://api.github.com/repos/MonkyOrg/Monky/releases/latest';
const UPDATER_PROCESS_NAME = 'monky-updater';
const AUTO_UPDATE_CRON = '0 4 * * *'; // daily at 4am

function getRepoRoot(): string {
  // The CLI is at apps/server/dist/cli.js, repo root is 3 levels up
...
async function checkForUpdate(): Promise<{ hasUpdate: boolean; local: string; remote: string; url: string }> {
...
function getUpdaterScriptPath(dataDir: string): string {
  return path.join(dataDir, 'auto-update.sh');
}

function generateUpdaterScript(repoRoot: string, dataDir: string): string {
...
async function enableAutoUpdate(dataDir: string): Promise<void> {
  ensurePm2();
