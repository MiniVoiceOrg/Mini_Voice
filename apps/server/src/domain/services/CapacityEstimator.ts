...
export class CapacityEstimator {
   ...
   */
  public static estimate(uploadMbps = 100, overrideCores?: number, overrideRamGb?: number): CapacityEstimate {
    const cpus = os.cpus();
    const cpuCores = overrideCores ?? (Array.isArray(cpus) && cpus.length > 0 ? cpus.length : 1);
    const ramTotalGb = overrideRamGb ?? (Math.round((os.totalmem() / (1024 * 1024 * 1024)) * 10) / 10);
    ...
    const maxAudioParticipants = Math.max(2, Math.min(maxAudioByBandwidth, maxAudioByCpu, maxAudioByRam));
