...
8. **Após o merge, aguarde a release ser gerada.**
   ...
   - A versão é calculada a partir da **última release publicada, betas inclusive**, avaliando apenas os commits feitos desde ela. Cada push move o número conforme o que carrega: `fix:` sobe patch, `feat:` sobe minor e breaking change sobe major, mesmo no meio de uma linha de beta (#378). Promover uma beta publica exatamente os números dela (`v3.4.2-beta001` → `v3.4.2`).
   - Betas saem como `v<MAJOR>.<MINOR>.<PATCH>-beta<NNN>` com 3 dígitos (`v3.0.0-beta001`). O zero à esquerda é obrigatório porque a página de releases do GitHub ordena pelo **nome da tag**: sem o padding, `beta9` apareceria depois de `beta14` (#338). O ponto não pode ser usado (`beta.014` é SemVer inválido). Como a base muda a cada release, o contador costuma ficar em `beta001` — ele só cresce quando a mesma base é publicada mais de uma vez.
