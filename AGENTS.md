...
8. **Após o merge, aguarde a release ser gerada.**
   ...
   - Betas saem como `v<MAJOR>.<MINOR>.<PATCH>-beta`, sem contador (`v3.1.2-beta`). O sufixo já carregou um número de 3 dígitos (`-beta001`) para que a página de releases do GitHub, que ordena pelo **nome da tag**, não enterrasse a build mais nova (#338); como a base passou a mudar a cada release (#378), o contador nunca saía de `001` e a ordenação passou a depender só de `MAJOR.MINOR.PATCH` (#382). As tags já publicadas (`-betaNNN` e o legado `-beta.N`) continuam sendo lidas normalmente.
